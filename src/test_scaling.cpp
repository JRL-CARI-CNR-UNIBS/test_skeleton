...
{
  ros::init(argc, argv, "node_test_scaling");
  ros::NodeHandle nh;
  ...
  for(int n_iter = init_test; n_iter<end_test; n_iter++)
  {
    ...
    start_conf = start_conf*3.14157/180;
    goal_conf = goal_conf*3.14157/180;
    ...
    pathplan::PathPtr path = trajectory->computePath(start_conf, goal_conf, solver, optimize_path);

    std::vector<pathplan::PathPtr> other_paths;

    pathplan::ReplannerManagerPtr replanner_manager = std::make_shared<pathplan::ReplannerManager>(path, other_paths, nh);

    sound_msg.sound=3;
    sound_pub.publish(sound_msg);

    for (int iiii=0;iiii<5;iiii++)
      ROS_INFO("LAUNCH ROSBAG!");
    ...
    if(!get_real_start_pos)
    {
      ...
      first=true;
      for(const std::pair<double,pathplan::PathPtr>& path_pair: path)
      {
